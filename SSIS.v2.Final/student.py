...
def check_ccode(coursecode):
    ...
    query = "SELECT coursecode FROM student WHERE LOWER(coursecode) = LOWER(%s)"
...
def add_student():
    ...
    if check_studentID(studentID) is True:
        ...
        if not course.check_course(coursecode):
            ...
            query = "INSERT INTO student (studentID, fullname, gender, yearlevel, coursecode) VALUES (%s, %s, %s, %s, %s)"
...
def view_students():
    ...
    print("ID Number, Name, Gender, Course Code, Year Level")
    for row in data:
...
def deleteByCourse(coursecode):
    cursor = db.cursor()
    query = "DELETE FROM student WHERE course = %s"
    cursor.execute(query, (coursecode,))
    db.commit()
...
def search_student():
    ...
    query = "SELECT * FROM student WHERE studentID LIKE %s OR fullname LIKE %s OR gender LIKE %s OR yearlevel LIKE %s OR coursecode LIKE %s"
